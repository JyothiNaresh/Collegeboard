...
(function ($) {
     ...
     ChiliBook = { //implied global
         ...
         ,
         selectionStyle: ["position:absolute; z-index:3000; overflow:scroll;", "width:16em;", "height:9em;", "border:1px solid gray;", "padding:15px;", "background-color:yellow;"].join(' ')
                     ...
                     while (step = steps[i++]) {
                         ...
                         if (aux[j]) {
                             ...
                             if ($.isFunction(step.replacement)) {
                                 ...
                                 replacement = step.replacement.apply({
                                     x: function () {
                                 ...
                                 replacement = step.replacement.replace(/(\\\$)|(?:\$\$)|(?:\$(\d+))/g, function (m, escaped, K) {
                                     if (escaped) { /* \$ */
                                         return "$";
                                     } else if (!K) { /* $$ */
                                         return filter(aux[j]);
