     ...
     $.tablesorter.addWidget({
         ...
         format: function (table) {
             ...
             if (!$t.hasClass('tablesorter-' + theme) || c.theme === theme || !table.hasInitialized) {
                 ...
                 .find('tfoot');
                 if (t.length) {
                     t.find('tr').addClass(o.footerRow)
         ...
         remove: function (table, c, wo) {
             var $t = $(table),
                 ...
                 rmv = o.sortNone + ' ' + o.sortDesc + ' ' + o.sortAsc;
             $t.removeClass('tablesorter-' + theme + ' ' + o.table)
                 .find(c.cssHeader).removeClass(o.header);
             $h.unbind('mouseenter mouseleave') // remove hover
 ...
 **************************/
             ...
             if (table.config.parsers && !$(table).hasClass('hasFilters')) {
                 ...
                 last = '', // save last filter search
                     ...
                     buildSelect = function (i, updating) {
                         ...
                         arry = (ts.sortText) ? arry.sort(function (a, b) {
                             return ts.sortText(table, a, b, i);
                         }) : arry.sort(true);
                 ...
                 .bind('addRows updateCell update appendCache search'.split(' ').join('.tsfilter '), function (e, filter) {
                     ...
                     if ((e.which < 32 && e.which !== 8) || (e.which >= 37 && e.which <= 40)) {
                         return;
                     }
                     // skip delay
                 ...
                 if (wo.filter_hideFilters === true) {
                     $t.find('.tablesorter-filter-row')
                         ...
                         st = setTimeout(function () {
                             // don't hide row if any filter has a value
                             if ($t.find('.' + (wo.filter_cssFilter || 'tablesorter-filter')).map(function () {
                                 return $(this).val() || '';
                             }).get().join('') === '') {
     ...
     $.tablesorter.addWidget({
         id: "stickyHeaders",
         format: function (table) {
             if ($(table).hasClass('hasStickyHeaders')) {
                 return;
             }
             ...
             $table.bind('sortEnd.tsSticky', function () {
             ...
             $table.after(t2);
             // make it sticky!
             win.bind('scroll.tsSticky', function () {
     ...
     $.tablesorter.addWidget({
         id: "resizable",
         format: function (table) {
             if ($(table).hasClass('hasResizable')) {
                 return;
             }
