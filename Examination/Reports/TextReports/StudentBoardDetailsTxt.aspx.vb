...
'   AUTHOR                            : JANGA REDDY
'   INITIAL BASELINE DATE             : 16-MAY-2015
...
Public Class StudentBoardDetailsTxt
...
#Region "Sql Query"
    Private Sub GetReportsDetails()
        Try
            ...
            'EXAM BRANCHES
            If LSTSelEBranch.Items.Count > 0 Then
                FilterString &= " AND ADMDT.EXAMBRANCHSLNO IN ("
                For I = 0 To LSTSelEBranch.Items.Count - 1
            ...
            'COURSE
            If LstSelCourse.Items.Count > 0 Then
                FilterString &= " AND ADMDT.COURSESLNO IN ("
                For I = 0 To LstSelCourse.Items.Count - 1
            ...
            'BOARD CODE
            If LSTSelBoardCode.Items.Count > 0 Then
                FilterString &= " AND brdcol.BOARDCOLLEGESLNO IN ("
                For I = 0 To LSTSelBoardCode.Items.Count - 1
            ...
            'BOARD DISTRICT
            If LSTSelBoardDist.Items.Count > 0 Then
                FilterString &= " AND brdcol.BOARDDISTSLNO IN ("
                For I = 0 To LSTSelBoardDist.Items.Count - 1
            ...
            DTStudent = "SELECT DISTINCT  ADMDT.ADMNO, BRDSTU.BOARDADMNO, BRDCOL.CODE || '-' || BRDCOL.NAME COLNAME,BRDSTU.BOARDNAME PUPILNAME, BRDSTU.FATHERNAME PARENTNAME, " & _
                             "BRDSTU.MOTHERNAME, BRDREG.NAME RELIGION, BRDCST.NAME CASTE,BRDSUBCST.NAME SUBCASTE, TO_CHAR (ADMDT.DOB, 'DD-MM-YYYY') DOB," & _
                             "DATEINWORDS (ADMDT.DOB, TO_CHAR (ADMDT.DOB, 'YYYY')) DOBWORDS,'ENGLISH' FIRTLANG, BRDSEC.NAME SECLANG, BRDMTG.NAME MOTHERTANGUE," & _
                             "TO_CHAR (BRDSTU.DATEOFJOIN, 'DD-MM-YYYY') ADMDT, 'GOOD' CONDUCT,BRDSTU.MOLE1, BRDSTU.MOLE2, BYP.NAME YEARPASS, CAT.NAME CATEXNAME,ADMDT.COURSESLNO, " & _
                             "BRDPH.NAME PH, BRDPE.NAME PRVEXAM, BRDSTU.PRV_HTNO,COURS.NAME JOINEDCOURSE, COURS.NAME COURSENAME,admDT.exambranchslno,EXAMBRANCH.EXAMBRANCHNAME EXAMBRANCHNAME " & _
                             "FROM EXAM_BOARDCOLLEGE_MT BRDCOL,EXAM_BOARDSTUDENT_DT BRDSTU,EXAM_BOARDRELIGION_MT BRDREG,EXAM_BOARDCASTE_MT BRDCST,EXAM_BOARDSUBCASTE_MT BRDSUBCST," & _
                             "T_ADM_DT ADMDT,EXAM_BOARDSECLANG_MT BRDSEC,EXAM_BOARDSECLANG_MT BRDMTG,T_DISTRICT_MT DIST,T_MANDAL_MT MAND,EXAM_BOARDYOP_MT BYP,EXAM_BOARDCATEOGORY_MT CAT," & _
                             "EXAM_BOARDPH_MT BRDPH,EXAM_BOARDPRVEXAM_MT BRDPE,T_COURSE_MT COURS,EXAM_EXAMBRANCH EXAMBRANCH , E_USER_BRANCH_ACADEMIC_MT EUA " & _
                             "WHERE ADMDT.UNIQUENO = BRDSTU.UNIQUENO AND BRDSTU.BOARDCOLLEGESLNO = BRDCOL.BOARDCOLLEGESLNO(+) AND BRDSTU.BOARDRELIGIONSLNO = BRDREG.BOARDRELIGIONSLNO(+) " & _
                             "AND BRDSTU.BOARDCASTESLNO = BRDCST.BOARDCASTESLNO(+) AND BRDSTU.BOARDSUBCASTESLNO = BRDSUBCST.BOARDSUBCASTESLNO(+) AND BRDSTU.BOARDSECLANGSLNO = BRDSEC.BOARDSECLANGSLNO(+) " & _
                             "AND BRDSTU.MOTHERTONGUESLNO = BRDMTG.BOARDSECLANGSLNO(+) AND DIST.DISTRICTSLNO = BRDCOL.DISTRICTSLNO AND MAND.MANDALSLNO = BRDCOL.MANDALSLNO AND BRDSTU.BOARDYOPSLNO = BYP.BOARDYOPSLNO " & _
                             "AND BRDSTU.BOARDCATEGORYSLNO = CAT.BOARDCATEGORYSLNO AND BRDSTU.BOARDPHSLNO = BRDPH.BOARDPHSLNO AND BRDSTU.BOARDPRVEXAMSLNO = BRDPE.BOARDPRVEXAMSLNO AND BRDSTU.JOINEDCOURSESLNO = COURS.COURSESLNO " & _
                             "AND ADMDT.EXAMBRANCHSLNO = EXAMBRANCH.EXAMBRANCHSLNO AND ADMDT.COMACADEMICSLNO =" & Session("COMACADEMICSLNO") & " " & FilterString & "  ORDER BY ADMDT.ADMNO"
...
#Region "Report String"
    ...
    Private Function FormatReportstring(ByVal Ds As DataSet) As StringBuilder
        Try
            ...
            Dim ireportwidth As Integer = 420
            ...
            For i = 0 To LSTSelEBranch.Items.Count - 1
                ...
                If DRW.Length > 0 Then
                    ...
                    DRC = Ds.Tables(0).Select("EXAMBRANCHSLNO='" & LSTSelEBranch.Items(i).Value.ToString & _
                                                    "' AND COURSESLNO='" & LstSelCourse.Items(0).Value.ToString & "'")
                    ...
                    If DRC.Length > 0 Then
                        ...
                        StrHead1 &= StrPadding("Board Code", 33, "M")
                        StrHead1 &= StrPadding("Student Name", 36, "L")
                        StrHead1 &= StrPadding("Father Name", 34, "L")
                        StrHead1 &= StrPadding("Mother Name", 34, "L")
                        StrHead1 &= StrPadding("Dob", 12, "M")
                        StrHead1 &= StrPadding("Religion", 10, "L")
                        StrHead1 &= StrPadding("Caste", 6, "L")
                        StrHead1 &= StrPadding("SubCaste", 20, "L")
                        StrHead1 &= StrPadding("Mothertangue", 13, "L")
                        StrHead1 &= StrPadding("Mole1", 45, "M")
                        StrHead1 &= StrPadding("Mole2", 45, "M")
                        StrHead1 &= StrPadding("Dateofjoin", 13, "L")
                        StrHead1 &= StrPadding("Joined Course", 16, "L")
                        StrHead1 &= StrPadding("1st Lang", 11, "L")
                        StrHead1 &= StrPadding("2nd Lang", 11, "L")
                        StrHead1 &= StrPadding("Conduct", 8, "L")
                        StrHead1 &= StrPadding("Yearpass", 12, "L")
                        StrHead1 &= StrPadding("Type Of Exam", 14, "L")
                        StrHead1 &= StrPadding("Phy-HandiC", 12, "L")
                        StrHead1 &= StrPadding("Prvexam", 14, "L")
                        StrHead1 &= StrPadding("Prv_htno", 12, "L")
                        StrHead1 &= StrPadding("Exam Branch Name", 18, "L")
                        StrHead1 &= StrPadding("Course", 8, "L")
                        'StrHead1 &= vbCrLf
                        ...
                        strfile.Append(PrintLines("-", ireportwidth))
                        For S As Integer = 0 To DRW.Length - 1

                            strfile = strfile.Append(StrPadding(S + 1, 6, "L"))
                            strfile.Append(StrPadding(DRW(S)("ADMNO").ToString, 9, "M"))
                            strfile.Append(StrPadding(DRW(S)("BOARDADMNO").ToString, 12, "M"))
                            strfile = strfile.Append(StrPadding(DRW(S)("COLNAME").ToString, 33, "M"))
                            strfile = strfile.Append(StrPadding(DRW(S)("PUPILNAME").ToString, 36, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("PARENTNAME").ToString, 34, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("MOTHERNAME").ToString, 34, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("DOB").ToString, 12, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("RELIGION").ToString, 10, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("CASTE").ToString, 6, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("SUBCASTE").ToString, 20, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("MOTHERTANGUE").ToString, 13, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("MOLE1").ToString, 45, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("MOLE2").ToString, 45, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("ADMDT").ToString, 13, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("JOINEDCOURSE").ToString, 16, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("FIRTLANG").ToString, 11, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("SECLANG").ToString, 11, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("CONDUCT").ToString, 8, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("YEARPASS").ToString, 12, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("CATEXNAME").ToString, 14, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("PH").ToString, 12, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("PRVEXAM").ToString, 14, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("PRV_HTNO").ToString, 12, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("EXAMBRANCHNAME").ToString, 18, "L"))
                            strfile = strfile.Append(StrPadding(DRW(S)("COURSENAME").ToString, 8, "L"))
