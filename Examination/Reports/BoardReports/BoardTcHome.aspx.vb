...
#Region "Grid Events"
    ...
    Private Sub dgGrid_DeleteCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgGrid.DeleteCommand
        Try
            ...
            If Tcgen = 1 Then
                Response.Redirect("BoardTCReport.aspx?ADMSLNO=" & dgGrid.Items(e.Item.ItemIndex).Cells(1).Text.ToString & "&UNIQUENO=" & dgGrid.Items(e.Item.ItemIndex).Cells(0).Text.ToString & "&BOARDCOLLEGESLNO=" & dgGrid.Items(e.Item.ItemIndex).Cells(2).Text.ToString & "&FLAG=" & Session("FLAG"))
            ElseIf Tcgen = 2 Then
                Response.Redirect("BoardTCReportts.aspx?ADMSLNO=" & dgGrid.Items(e.Item.ItemIndex).Cells(1).Text.ToString & "&UNIQUENO=" & dgGrid.Items(e.Item.ItemIndex).Cells(0).Text.ToString & "&BOARDCOLLEGESLNO=" & dgGrid.Items(e.Item.ItemIndex).Cells(2).Text.ToString & "&FLAG=" & Session("FLAG"))
            End If
...
#Region "Scrub"
    ...
    'SELECT TO_CHAR(TO_DATE('01-JAN-2008', 'DD-MON-YYYY'), 'DDSP-MONTH-YYYYSP') FROM dual;   TO_CHAR(DATE,'DDSP-MONTH-YYYYSP')
