...
#Region "Fill Methods"
    ...
    Private Function FilldataGrid(ByVal PageIndex)
        Try
            ...
            Dim SqlStr As String = "SELECT UC.USERSLNO,UC.COMACADEMICSLNO,UC.COMBINATIONKEY,EM.EXAMNAME FROM EXAM_KEYOBJ_COMB_MAP UC,EXAM_EXAMNAME EM " & _
    ...
    Private Sub dgVusers_DeleteCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.DataGridCommandEventArgs) Handles dgVusers.DeleteCommand
        Try
            ...
            If UserSlno = 1 Then
                ...
                Exit Sub
            End If
            RtnStr = objUcls.CombinationsKeyObjections_Delete(UserSlno, Val(dgVusers.Items(e.Item.ItemIndex).Cells(2).Text), Val(dgVusers.Items(e.Item.ItemIndex).Cells(3).Text))
            If RtnStr = 1 Then
                FilldataGrid(0)
...
#Region "Image Buttons"
    ...
    Private Sub ibtnBatch_Click(ByVal sender As System.Object, ByVal e As System.Web.UI.ImageClickEventArgs) Handles ibtnBatch.Click
        Try
            Response.Redirect("KeyObj_Combinations.aspx?USERTYPESLNO=" & DrpUserTypes.SelectedValue.ToString)
        Catch Oex As OracleException
            StartUpScript("", DataBaseErrorMessage(Oex.Message))
        Catch ex As Exception
            If GeneralErrorMessage(ex.Message) <> "" Then
